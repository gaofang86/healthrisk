...
const COMMUNITY_RESOURCES = [
  ...
  },
  {
    ...
    name: "Ratna Juwita",
    area: "RT 11",
    type: "Larvicide / prevention kits",
      ...
      >
        ...
        >
          <div style={{ fontSize: 12, color: "#666", marginBottom: 8 }}>
            Choose a resource provider to match:
          </div>
          {resources.map((r) => (
            <div
              key={r.id}
              ...
              style={{
                ...
                padding: "8px 10px",
                borderRadius: 8,
                border: `0.5px solid ${selectedResource?.id === r.id ? "#1D9E75" : "#eee"}`,
                background:
                  selectedResource?.id === r.id ? "#E1F5EE" : "transparent",
                cursor: "pointer",
                marginBottom: 6,
              }}
            >
        ...
        {!result && (
          <button
            ...
            style={{
              width: "100%",
              padding: "9px",
              background: selectedResource ? "#1D9E75" : "#eee",
              color: selectedResource ? "#fff" : "#aaa",
              border: "none",
              borderRadius: 8,
              fontSize: 13,
          ...
          >
            {loading
              ? "Generating coordination advice…"
              : "Generate match advice with AI"}
          </button>
        )}
        {error && (
          <div
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          padding: "8px 18px",
          background: question.trim() ? "#185FA5" : "#eee",
...
export default function App() {
    ...
    Papa.parse("/test_predictions.csv", {
      ...
      complete: (results) => {
        ...
        const uniqueCountries = [
          ...new Set(data.map((r) => r.adm_0_name)),
        ].sort();
        ...
        if (indonesiaWeeks.length > 0) {
          ...
          const row = data.find(
            (r) =>
              r.adm_0_name === "INDONESIA" && r.iso_week === indonesiaWeeks[0],
          );
          if (row) {
  ...
  function handleCountryChange(country) {
    ...
    if (countryWeeks.length > 0) {
      ...
      const row = predictions.find(
        (r) => r.adm_0_name === country && r.iso_week === countryWeeks[0],
      );
      if (row) {
        const prob = parseFloat(row.pred_prob);
        setPredProb(prob);
  ...
  function handleWeekChange(week) {
    ...
    const row = predictions.find(
      (r) => r.adm_0_name === selectedCountry && r.iso_week === week,
    );
    if (row) {
      const prob = parseFloat(row.pred_prob);
      setPredProb(prob);
        ...
        {navItems.map((n) => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: "none",
              background: activePage === n.id ? "#f5f5f5" : "transparent",
              cursor: "pointer",
              fontSize: 13,
              color: activePage === n.id ? "#111" : "#666",
              fontWeight: activePage === n.id ? 600 : 400,
              fontFamily: "inherit",
              textAlign: "left",
              marginBottom: 2,
            }}
      ...
      >
          ...
          (() => {
            ...
            const actionsByTier = {
              low: [
                {
                  icon: "🪣",
                  text: "Empty any standing water around your home — even bottle caps matter",
                },
                {
                  icon: "🦟",
                  text: "Use mosquito repellent when outdoors, especially at dawn and dusk",
                },
                {
                  icon: "🌿",
                  text: "Keep your surroundings clean — cut grass and remove debris",
                },
              ],
              moderate: [
                {
                  icon: "🪣",
                  text: "Check and empty ALL water containers within 500m urgently",
                },
                {
                  icon: "💊",
                  text: "Stock oral rehydration salts at home — available at Puskesmas",
                },
                {
                  icon: "🩺",
                  text: "If fever appears, go to clinic within 24 hours — do not wait",
                },
                {
                  icon: "📢",
                  text: "Tell your neighbors — community action cuts risk fastest",
                },
              ],
              high: [
                {
                  icon: "🏥",
                  text: "Go to Puskesmas immediately if you have fever + rash or muscle pain",
                },
                {
                  icon: "💧",
                  text: "Stay hydrated — drink at least 2L water daily if feeling unwell",
                },
                {
                  icon: "🚨",
                  text: "Call 119 ext 8 (Indonesia) if symptoms worsen rapidly",
                },
                {
                  icon: "👴",
                  text: "Check on elderly or sick neighbors today — they need you",
                },
              ],
              critical: [
                {
                  icon: "🚨",
                  text: "Seek medical care NOW if you have any fever — do not delay",
                },
                {
                  icon: "📞",
                  text: "Call 119 ext 8 immediately for emergency health assistance",
                },
                {
                  icon: "🏥",
                  text: "Nearest hospital: RSUD district hospital — bring BPJS card if available",
                },
                {
                  icon: "🤝",
                  text: "Post on the needs board if you cannot travel — community will help",
                },
              ],
            };
            const resourcesByCountry = {
              INDONESIA: [
                {
                  type: "🏥",
                  name: "Puskesmas (Community Health Center)",
                  detail: "Free primary care · bring KTP",
                  link: null,
                },
                {
                  type: "🏨",
                  name: "RSUD District Hospital",
                  detail: "Emergency & dengue ward · bring BPJS card",
                  link: null,
                },
                {
                  type: "📞",
                  name: "National Health Hotline",
                  detail: "119 ext 8 · 24 hours",
                  link: null,
                },
                {
                  type: "🌐",
                  name: "World Mosquito Program",
                  detail: "Wolbachia mosquito release program",
                  link: "https://www.worldmosquitoprogram.org",
                },
              ],
              COLOMBIA: [
                {
                  ...
                  detail: "Primary care · bring cédula",
                  link: null,
                },
                {
                  type: "📞",
                ...
                },
                {
                  type: "🌐",
                  name: "INS Colombia",
                  detail: "National Institute of Health",
                  link: "https://www.ins.gov.co",
                },
              ],
              PERU: [
                {
                  type: "🏥",
                  name: "Centro de Salud MINSA",
                  detail: "Free primary care · bring DNI",
                  link: null,
                },
                {
                  type: "📞",
                  name: "SAMU Peru",
                  detail: "106 · emergency ambulance",
                  link: null,
                },
                {
                  type: "🌐",
                  name: "MINSA Peru",
                  detail: "Ministry of Health · dengue alerts",
              ...
              BOLIVIA: [
                ...
                },
                {
                  type: "📞",
                  name: "SEDES",
                  detail: "Departmental health emergency line",
                  link: null,
                },
              ],
              PANAMA: [
                ...
                },
                {
                  type: "📞",
                  name: "MINSA Panama",
                  detail: "169 · health emergency",
                  link: null,
                },
                {
                  type: "🌐",
                  name: "MINSA Panama",
                  detail: "Ministry of Health",
                  link: "https://www.minsa.gob.pa",
                },
              ],
            };
            const supportLinks = [
              {
                icon: "❤️",
                label: "Donate to MSF",
                sub: "Dengue & tropical disease response",
              ...
              },
              {
                icon: "🌍",
                label: "Support WHO",
                sub: "Global dengue elimination program",
              ...
              {
                icon: "💼",
                label: "Health sector jobs",
                sub: "ReliefWeb · humanitarian roles",
              ...
              },
              {
                icon: "🤝",
                label: "Volunteer",
                sub: "UN Volunteers · community health",
            ...
            const resources =
              resourcesByCountry[selectedCountry] ||
              resourcesByCountry["INDONESIA"];
            ...
            const maxProb = Math.max(
              ...recentRows.map((r) => parseFloat(r.pred_prob)),
              THRESHOLD * 4,
            );
            const dengueFacts = [
              {
                ...
                body: "Sudden high fever (39–40°C), severe headache, pain behind the eyes, muscle and joint pain. Symptoms appear 4–10 days after a mosquito bite.",
              },
              {
                icon: "🦟",
                title: "How it spreads",
                body: "Transmitted only by the Aedes aegypti mosquito — not person to person. These mosquitoes bite during the day, especially at dawn and dusk.",
              },
              {
                icon: "⚠️",
                title: "Warning signs",
                body: "Severe dengue can be life-threatening. Watch for: stomach pain, persistent vomiting, bleeding gums, fatigue — seek emergency care immediately.",
              },
              {
                icon: "💉",
                title: "Treatment",
                body: "No specific antiviral treatment exists. Rest, fluids, and paracetamol for fever. Avoid ibuprofen and aspirin. Most people recover in 1–2 weeks.",
              },
            ];
            return (
              <div>
                ...
                <div>
                  ...
                  >
                    Donate, volunteer, or explore opportunities in global health
                  </div>
                  <div
                    style={{
                      display: "grid",
        ...
        {activePage === "needs" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>
              Prioritized by vulnerability · click "Match" to get AI
              coordination advice
            </div>
            <div
              style={{
        ...
        {activePage === "resources" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>
              Tell the community what you can offer — your help could reach
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 13, color: "#3B6D11", lineHeight: 1.7 }}
                  >
                    Community resources close gaps that government programs
                    can't fill — faster, cheaper, and with more trust. A single
                    car ride or bag of rice can change someone's week.
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  {[
                    {
                      icon: "🚗",
                      label: "Car transport to clinic",
                      count: "2 open requests",
                    },
                    {
                      icon: "🍚",
                      label: "Food & ORS supplies",
                      count: "1 open request",
                    },
                    {
                      icon: "👁️",
                      label: "Check-in for elderly",
                      count: "1 open request",
                    },
                    {
                      icon: "🦟",
        ...
        {activePage === "post" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>
              Ask the community for help — requests are matched privately and
              with care.
            </div>
            <div
              style={{
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 13, color: "#185FA5", lineHeight: 1.7 }}
                  >
                    Your request is shown anonymously on the needs board.
                    Community members who can help will reach out. Government
                    resources fill gaps when community supply runs out.
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  {[
                    ...
                    {
                      icon: "🟡",
                      label: "Unemployed / low income",
                      sub: "Matched third",
                    },
                    {
                      icon: "🔵",
                      label: "No BPJS / insurance",
                      sub: "Guided to resources",
                    },
                  ].map((item, i) => (
                    <div
                      key={i}
        ...
        {activePage === "ask" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>
              Powered by Groq · llama-3.3-70b · responds in your language
            </div>
            ...
            return (
              <div>
                ...
                >
                  {[
                    ...
                    },
                    {
                      label: "Coverage rate",
                      value: gm.coverage,
                      sub: "by community alone",
                      vc: riskIdx >= 2 ? "#A32D2D" : "#3B6D11",
                    },
                    {
                      label: "Gov budget left",
                      value: gm.budgetLeft,
                      sub: "of $50,000",
                    },
                  ].map((m, i) => (
                ...
                {gm.alert && (
                  <div
                    style={{
                      background: riskIdx >= 3 ? "#FCEBEB" : "#FAEEDA",
                      border: `0.5px solid ${riskIdx >= 3 ? "#F09595" : "#EF9F27"}`,
                      borderRadius: 8,
                      padding: "12px 16px",
                      fontSize: 13,
                      color: riskIdx >= 3 ? "#A32D2D" : "#854F0B",
                      marginBottom: 20,
                    }}
                  >
                      ...
                      {[
                        ...
                        <div
                          ...
                          style={{
                            ...
                            gap: 10,
                            padding: "8px 0",
                            borderBottom: i < 3 ? "0.5px solid #eee" : "none",
                          }}
                        >
          ...
          (() => {
            const allocationByTier = {
              low: {
                ...
                items: [
                  {
                    name: "Routine larvicide — high-density areas",
                    impact: "Prevention",
                    cost: "$1,200",
                    badge: "Routine",
                    bc: "#EAF3DE",
                  ...
                  {
                    name: "Community health education sessions",
                    impact: "Awareness",
                    cost: "$800",
                    badge: "Routine",
                    bc: "#EAF3DE",
                  ...
                  {
                    name: "Reserve — emergency response fund",
                    impact: "–",
                    cost: "$48,000",
                    badge: "Reserve",
                    bc: "#f5f5f5",
              ...
              moderate: {
                ...
                items: [
                  {
                    name: "Targeted larvicide — RT 09, 12, 15",
                    impact: "40% risk cut",
                    cost: "$3,000",
                    badge: "High impact",
                    bc: "#EAF3DE",
                  ...
                  {
                    name: "Volunteer check-in — elderly residents",
                    impact: "5 elderly",
                    cost: "$1,800",
                    badge: "High impact",
                    bc: "#EAF3DE",
              ...
              high: {
                ...
                items: [
                  {
                    name: "Emergency BPJS enrollment — Keluarga Eko",
                    impact: "Covers 4 people",
                    cost: "$200",
                    badge: "Urgent",
                    bc: "#FCEBEB",
                    bt: "#A32D2D",
                  ...
                  {
                    name: "Volunteer check-in — elderly in RT 09, 12, 15",
                    impact: "5 elderly",
                    cost: "$1,800",
                    badge: "High impact",
                    bc: "#EAF3DE",
                  ...
                  {
                    name: "Community-wide larvicide spraying",
                    impact: "60% risk cut",
                    cost: "$6,000",
                    badge: "High impact",
                    bc: "#EAF3DE",
                  ...
                  {
                    name: "Mosquito net distribution — low income households",
                    impact: "18 families",
                    cost: "$900",
                    badge: "Medium",
                    bc: "#FAEEDA",
                  ...
                  {
                    name: "Reserve — new needs this week",
                    impact: "–",
                    cost: "$42,100",
                    badge: "Reserve",
                    bc: "#f5f5f5",
              ...
              critical: {
                ...
                items: [
                  {
                    name: "Emergency hospital capacity — RSUD surge support",
                    impact: "Critical",
                    cost: "$8,000",
                    badge: "🚨 Critical",
                    bc: "#FCEBEB",
                  ...
                  {
                    name: "Mass larvicide + fogging — all RTs",
                    impact: "75% risk cut",
                    cost: "$6,000",
                    badge: "🚨 Critical",
                    bc: "#FCEBEB",
                  ...
                  {
                    name: "Emergency BPJS enrollment — all uninsured",
                    impact: "~12 people",
                    cost: "$600",
                    badge: "Urgent",
                    bc: "#FCEBEB",
                  ...
                  {
                    name: "Food + medicine distribution — low income",
                    impact: "25 families",
                    cost: "$2,500",
                    badge: "Urgent",
                    bc: "#FCEBEB",
                  ...
                  {
                    name: "CHW deployment — all gap areas",
                    impact: "RT 09,12,15",
                    cost: "$1,900",
                    badge: "High impact",
                    bc: "#EAF3DE",
            ...
            return (
              <div>
                ...
                <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>
                  Knapsack-optimized · {selectedCountry} ·{" "}
                  {riskLevel.toUpperCase()} risk week · gaps not covered by
                  community
                </div>
                <div
                  style={{
                    background: riskIdx >= 3 ? "#FCEBEB" : "#f5f5f5",
                    borderRadius: 10,
                    padding: "14px 18px",
                    marginBottom: 20,
                    border: riskIdx >= 3 ? "0.5px solid #F09595" : "none",
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    <span style={{ color: "#666" }}>
                      Budget deployed this week
                    </span>
                    <span style={{ fontWeight: 700 }}>
                      ${alloc.used.toLocaleString()} / $50,000
                    </span>
                  </div>
                  <div
                    style={{
        ...
        {activePage === "gov-gaps" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>
              Needs that community resources cannot meet
            </div>
            <div
              style={{
                background: "#FCEBEB",
            ...
            >
              ▲ RT 09, 12, 15 have zero community resource providers — highest
              priority for government deployment.
            </div>
            {[
              {
                priority: "elderly",
                label: "Unmatched · Elderly",
                name: "Mbah Sutarni — no volunteer match",
                detail:
                  "Community has no available regular check-in volunteer for RT 09. Government volunteer program or CHW assignment needed.",
              },
              {
                priority: "unemployed",
                label: "Unmatched · Low income",
                name: "Keluarga Eko — no BPJS, no food bank nearby",
                detail:
                  "Food bank in RT 07 is too far. Not enrolled in PKH. BPJS PBI enrollment pending.",
              },
            ].map((n, i) => {
              const pc = PRIORITY_COLORS[n.priority];
              return (
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4 }}>
                Systematic gap: 3 RTs with no community resource providers
              </div>
              <div style={{ fontSize: 13, color: "#666" }}>
                Recommend: kader recruitment drive in RT 09, 12, 15. Estimated
                cost: $1,800/month for 3 part-time kaders.
              </div>
            </div>
          </div>
        )}
